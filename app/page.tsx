    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#090d16', color: '#f1f5f9', padding: '40px 20px', fontFamily: 'system-ui, sans-serif', direction: 'rtl', display: 'flex', flexDirection: 'column' }}>
      <div style={{ maxWidth: '850px', margin: '0 auto', flex: 1, width: '100%' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {fetching && events.length === 0 ? (
                <tr>
                  <td colSpan={3} style={{ padding: '30px', textAlign: 'center', color: '#64748b' }}>جاري التحميل...</td>
                </tr>
              ) : filteredEvents.length === 0 ? (
                <tr>
                  <td colSpan={3} style={{ padding: '30px', textAlign: 'center', color: '#64748b' }}>لا توجد مناسبات مضافة حالياً</td>
                </tr>
              ) : (
        ...
        <form onSubmit={handleSaveEvent} style={{ backgroundColor: '#111827', padding: '28px', borderRadius: '16px', border: '1px solid #1f2937', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.3)' }}>
          <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#f8fafc', marginBottom: '20px' }}>إضافة مناسبة جديدة</h3>
          ...
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '14px', backgroundColor: '#0284c7', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: '700', fontSize: '15px', cursor: 'pointer' }}>
            {loading ? 'جاري الحفظ...' : 'إضافة المناسبة'}
          </button>
        </form>
      </div>
